...
import org.apache.spark.sql.mqtt.provider.HDFSMQTTSourceProvider._
...
class HDFSBasedMQTTStreamSource(
  ...
  private val hadoopConfig: Configuration = HDFSBasedMQTTStreamSource.hadoopConfig match {
    case null =>
    ...
    withLock[Option[Offset]] {
      ...
      val result = if (!hasNewMessageForCurrentBatch()) {
        if (lastOffset == -1) {
          ...
          Some(org.apache.spark.sql.mqtt.offset.LongOffset(lastOffset))
        }
      } else {
  ...
  override def getBatch(start: Option[Offset], end: Offset): DataFrame = {
    ...
    sqlContext.internalCreateDataFrame(rdd, MQTTRecordToRowConvertor.schema, true)
  }
  ...
  private def getOffsetValue(offset: Offset): Long = {
    val offsetValue = offset match {
      case o: org.apache.spark.sql.mqtt.offset.LongOffset => o.offset
